...
graph TD
    subgraph TODAY["Today: Static Architecture"]
        ...
        T_SVC --> T_DB["Database"]
        T_NOTE["Every path is coded<br/>Every edge case is a branch<br/>Changes require deploys"]
    end
    ...
    subgraph FUTURE["Future: Adaptive Architecture"]
        ...
        F_NOTE["Paths emerge from reasoning<br/>Novel inputs handled without new code<br/>Behavior changes with prompts"]
    end
...
graph LR
    subgraph LAYERS[""]
        direction TB
        L1["FRONTEND<br/>──────<br/>Forms and buttons<br/>        becomes<br/>Intent interfaces<br/>Streaming agent UIs<br/>Conversational + visual hybrid"]
        L2["API LAYER<br/>──────<br/>Fixed REST endpoints<br/>        becomes<br/>Agent router<br/>Semantic dispatch<br/>Dynamic orchestration"]
        L3["BUSINESS LOGIC<br/>──────<br/>If-else trees, sagas<br/>        becomes<br/>Reasoning loops<br/>Planned sequences<br/>Reflective self-correction"]
        L4["DATA LAYER<br/>──────<br/>SQL, ORM queries<br/>        becomes<br/>Natural language to SQL<br/>Semantic search<br/>Multi-modal retrieval"]
        L5["OPERATIONS<br/>──────<br/>Static alerts, runbooks<br/>        becomes<br/>Agent SRE<br/>Anomaly reasoning<br/>Autonomous remediation"]
    end

    style LAYERS fill:#e0e0e0,stroke:#424242
```
...
graph LR
    subgraph LIFECYCLE[""]
        direction TB
        DEV["Development<br/>──────<br/>Write prompts + tools<br/>instead of logic branches<br/>Test with eval suites"]
        DEPLOY["Deployment<br/>──────<br/>Deploy agent configs<br/>Hot-swap prompts<br/>A/B test behaviors"]
        MONITOR["Monitoring<br/>──────<br/>Trace every reasoning step<br/>Cost per interaction<br/>Quality scoring"]
        ITERATE["Iteration<br/>──────<br/>Update prompts, not code<br/>Add tools, not endpoints<br/>Improve via reflection data"]
    end

    DEV --> DEPLOY --> MONITOR --> ITERATE --> DEV

    style LIFECYCLE fill:#e0e0e0,stroke:#424242
```
...
graph TB
    subgraph EASIER["Becomes Easier"]
        E1["Handling novel inputs"]
        E2["Cross-service orchestration"]
        E3["Natural language interfaces"]
        E4["Adaptive error recovery"]
        E5["Rapid prototyping"]
    end

    subgraph HARDER["Becomes Harder"]
        H1["Deterministic behavior"]
        H2["Cost prediction"]
        H3["Latency guarantees"]
        H4["Security boundary enforcement"]
        H5["Debugging production issues"]
        H6["Compliance and audit"]
    end
...
graph TD
    subgraph NOW["Today: Fragmented"]
        N1["OpenAI Function Calling"]
        N2["Anthropic Tool Use"]
        N3["Custom integrations"]
        N4["No interoperability"]
    end

    subgraph EMERGING["Emerging: MCP"]
        M1["Model Context Protocol"]
        M2["Standard tool interface"]
        M3["Tool servers as services"]
        M4["Any LLM connects to any tool"]
    end

    subgraph FUTURE["Future: Agent Protocol"]
        F1["Agent-to-agent communication standard"]
        F2["Capability advertisement"]
        F3["Trust and delegation chains"]
        F4["Agent marketplace / registry"]
    end
...
```mermaid
graph TD
    subgraph TEAM["Development Team Composition"]
        direction LR
        HUMAN_DEV["Human Engineers<br/>──────<br/>System design<br/>Critical decisions<br/>Agent supervision<br/>Prompt engineering"]
        
        AGENT_DEV["Agent Engineers<br/>──────<br/>Coding agents<br/>Testing agents<br/>Review agents<br/>Deploy agents"]
        
        AGENT_OPS["Agent Ops<br/>──────<br/>SRE agents<br/>Monitoring agents<br/>Incident response agents"]
    end

    HUMAN_DEV -->|design + supervise| AGENT_DEV
    HUMAN_DEV -->|configure + audit| AGENT_OPS

    style TEAM fill:#e0e0e0,stroke:#424242
